...
import 'package:imarketku/services/eCommerce/cart_service/cart_service.dart';
...
class CartController extends StateNotifier<CartState> {
  ...
  Future<void> increment({required int productId}) async {
    try {
      ...
      final response = await ref
          .read(cartServiceProvider)
          .increentQty(productId: productId);
      if (response.statusCode == 200) {
        final List<dynamic> data = response.data['data']['cart_items'];
        _cartItems =
            data.map((cartItem) => CartItem.fromJson(cartItem)).toList();
...
class CartState {
  ...
  CartState({required this.isLoading, required this.cartItems});
}
