...
import 'package:imarketku/generated/intl/messages_ar.dart';
import 'package:imarketku/models/eCommerce/common/common_response.dart';
import 'package:imarketku/models/eCommerce/order/add_product_review_model.dart';
import 'package:imarketku/models/eCommerce/order/buy_now_order_place.dart';
...
import 'package:imarketku/services/eCommerce/order/order_service.dart';
import 'package:imarketku/utils/global_function.dart';
import 'package:imarketku/utils/request_handler.dart';
...
class OrderController extends StateNotifier<bool> {
  ...
  Future<CommonResponse> placeOrder({
    required OrderPlaceModel orderPlaceModel,
  }) async {
    try {
      state = true;
  ...
  Future<CommonResponse> buyNow({
    required BuyNowOrderPlace orderPlaceModel,
  }) async {
    try {
      state = true;
      final response = await ref
  ...
  Future<void> getOrders({
    ...
    try {
      ...
      final response = await ref
          .read(orderServiceProvider)
          .getOrders(orderStatus: orderStatus, page: page, perPage: perPage);
  ...
  Future<CommonResponse> addProudctReview({
    required AddProductReviewModel productReviewModel,
  }) async {
    try {
      state = true;
      final response = await ref
...
class OrderDetailsController extends StateNotifier<AsyncValue<OrderDetails>> {
  ...
  OrderDetailsController(this.ref) : super(const AsyncValue.loading());
