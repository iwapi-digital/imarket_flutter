...
import 'package:imarketku/controllers/eCommerce/voucher/voucher_controller.dart';
...
import 'package:imarketku/models/eCommerce/voucher/voucher_apply_model.dart';
...
final priceRangeProvider = StateProvider<RangeValues>((ref) {
  ...
  return RangeValues(filter?.minPrice ?? 0.0, filter?.maxPrice ?? 5000.0);
});
  ...
  calculateSummery({String? couponCode}) async {
    ...
    for (var product in ref.read(finalCheckoutProducts)) {
      ...
      if (product.discountPrice > 0) {
        discount +=
            (product.currentPrice - product.discountPrice) *
            product.productsQTY;
      }
    ...
    if (products.isNotEmpty &&
      ...
      final VoucherApplyModel voucherApplyModel = VoucherApplyModel(
        couponCode,
        products,
      );
      ref
          ...
          .applyVoucher(
            voucherApplyModel: voucherApplyModel,
            showSnakbar: false,
          )
          .then((couponDiscunt) {
