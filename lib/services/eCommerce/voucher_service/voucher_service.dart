...
import 'package:imarketku/services/base/eCommerce/voucher_provider_base.dart';
...
class VoucherService implements VoucherProviderBase {
  ...
  Future<Response> getVouchers({required int shopId}) async {
    final response = await ref
        ...
        .get(AppConstants.getVoucher, query: {'shop_id': shopId});
    return response;
  }
  ...
  Future<Response> collectVoucher({required int couponCode}) async {
    final response = await ref
        .read(apiClientProvider)
        .post(AppConstants.collectVoucher, data: {'coupon_id': couponCode});
    return response;
  }
