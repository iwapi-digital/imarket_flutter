...
import 'package:imarketku/services/base/eCommerce/cart_provider_base.dart';
...
class CartService implements CartProviderBase {
  ...
  Future<Response> addToCart({required AddToCartModel addToCartModel}) async {
    final response = await ref
        ...
        .post(AppConstants.addToCart, data: addToCartModel.toMap());
    return response;
  }
  ...
  Future<Response> decrementQty({required int productId}) async {
    final response = await ref
        .read(apiClientProvider)
        .post(AppConstants.decrementQty, data: {'product_id': productId});
    return response;
  }
  ...
  Future<Response> deleteGiftFromCart({required int giftId}) async {
    final response = await ref
        .read(apiClientProvider)
        .delete(AppConstants.removeGift, query: {'cart_id': giftId});
