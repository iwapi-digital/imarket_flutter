...
import 'package:imarketku/services/base/eCommerce/return_product_base.dart';
import 'package:imarketku/utils/api_client.dart';

final returnProductServiceProvider = Provider(
  (ref) => ReturnProductServiceImpl(ref),
);
...
class ReturnProductServiceImpl implements ReturnProductService {
  ...
  Future<Response> submitReturnProduct({
    ...
    final response = await ref
        ...
        .post(AppConstants.returnOrderSubmit, data: returnOrder.toMap());
    return response;
  }
  ...
  Future<Response> getReturnOrderDetails({required int orderId}) async {
    final response = await ref
        .read(apiClientProvider)
        .get('${AppConstants.returnOrderDetails}/$orderId');
    return response;
  }
}
