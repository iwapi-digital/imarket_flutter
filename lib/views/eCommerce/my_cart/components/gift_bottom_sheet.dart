...
import 'package:imarketku/components/ecommerce/custom_text_field.dart';
...
import 'package:imarketku/models/eCommerce/cart/gift.dart';
import 'package:imarketku/models/eCommerce/cart/gift_add_model.dart';
import 'package:imarketku/models/eCommerce/cart/gift_item.dart';
...
class _GiftBottomSheetState extends ConsumerState<GiftBottomSheet> {
  ...
  @override
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) => _handleExistingData());
    super.initState();
  }
  ...
  void _handleRemoveTap() async {
    await ref
        .read(cartController.notifier)
        .deleteGiftFromCart(giftId: widget.gift!.cartId)
        .then((value) {
          _cleanAll();
          context.nav.pop();
        });
        ...
        Row(
          children: [
            Text(
              ...
              ).bodyText.copyWith(fontSize: 16.sp, fontWeight: FontWeight.w400),
            ),
            Gap(5.w),
            Text(
              "( Optional )",
              style: AppTextStyle(
                GlobalFunction.navigatorKey.currentContext!,
              ).bodyTextSmall.copyWith(fontSize: 10.sp),
            ),
          ],
        ),
        Gap(4.h),
  ...
  void showAddressModalBottomSheet(BuildContext context) {
    showModalBottomSheet(
      ...
      context: context,
      builder: (context) => const AddressModalBottomSheet(isGift: true),
    );
  }
  ...
  Widget buildGiftItem({required ShopGift item, required int index}) {
    return Consumer(
      builder: (context, ref, _) {
        ...
        return InkWell(
          onTap:
              () => ref.read(selectedGiftIdProvider.notifier).state = item.id,
          child: Container(
