...
class OrderDetailsCard extends ConsumerStatefulWidget {
  ...
  const OrderDetailsCard({super.key, required this.orderDetails});
...
class _OrderDetailsCardState extends ConsumerState<OrderDetailsCard> {
  ...
  static final isDownloadPaymentReceiptloading = StateProvider<bool>(
    (ref) => false,
  );
        ...
        children: [
          ...
          _buildRowWidget(
            ...
            value: DateFormat(
              'd MMMM y',
              'en_US',
            ).format(DateTime.parse(widget.orderDetails.data.order.createdAt)),
          ),
          Gap(14.h),
  ...
  Widget _buildRowWidget({
    ...
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(key, style: AppTextStyle(context).bodyTextSmall),
        if (isOrderStatus) ...[
  ...
  Widget _buildPaymentStatusWidget() {
    return Container(
      ...
      decoration: BoxDecoration(
        color:
            widget.orderDetails.data.order.paymentStatus == 'Pending'
      ...
      child: Center(
        child: Text(
          ...
          style: AppTextStyle(
            ContextLess.context,
          ).bodyText.copyWith(fontSize: 12.sp, color: EcommerceAppColor.white),
        ),
      ),
    );
  }
