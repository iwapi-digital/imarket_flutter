...
import 'package:imarketku/views/eCommerce/order_details/components/product_review_dialog.dart';
...
class _OrderProductCardState extends State<OrderProductCard> {
      ...
      child: Material(
        ...
        child: Container(
          decoration: BoxDecoration(
            border: Border(
              top: BorderSide(color: colors(context).accentColor!, width: 2.0),
            ),
          ),
        ...
        return Row(
          children: [
            Text(
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              "${widget.product.orderQty} x  ${GlobalFunction.price(ref: ref, price: widget.product.discountPrice != 0 ? widget.product.discountPrice.toString() : widget.product.price.toString())} ",
  ...
  Widget _buildReviewButton({required BuildContext context}) {
    return Material(
  ...
  _buildSelectedIndicator(bool isSelected) {
    return Padding(
      ...
      child:
          isSelected
              ? Image.asset(
                "assets/png/return_selected.png",
                width: 24.w,
                height: 24.w,
              )
              : Container(
