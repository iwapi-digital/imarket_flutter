...
import 'package:imarketku/models/eCommerce/common/product_filter_model.dart';
...
class FilterModalBottomSheet extends ConsumerStatefulWidget {
  ...
  FilterModalBottomSheet({super.key, required this.productFilterModel});
...
class _FilterModalBottomSheetState
  ...
  final EdgeInsets _edgeInsets = EdgeInsets.symmetric(
    horizontal: 20.w,
    vertical: 10.h,
  ...
  Widget _buildHeader() {
    return Row(
      ...
      children: [
        Text(S.of(context).filter, style: AppTextStyle(context).subTitle),
        IconButton(
          onPressed: () {
            _onPressClear();
            context.nav.pop();
    ...
    return Wrap(
      ...
      children: List.generate(
        ...
        (index) => Material(
          ...
          color:
              ref.watch(selectedReviewIndex) == index
      ...
      children: List.generate(
        ...
        (index) => Padding(
          ...
          child: ChoiceChip(
            ...
            checkmarkColor: colors(context).light,
            selected:
                ref.watch(selectedCategoryByIndex) != null &&
                ref.watch(selectedCategoryByIndex) == index,
            onSelected: (bool selected) {
      ...
      children: List.generate(
        ...
        (index) => Padding(
          ...
          child: ChoiceChip(
            ...
            label: Text(brands?[index].name ?? ''),
            selectedColor: colors(context).primaryColor,
            surfaceTintColor: colors(context).light,
            checkmarkColor: colors(context).light,
            selected:
                ref.watch(selectedBrandIndex) != null &&
                ref.watch(selectedBrandIndex) == index,
            onSelected: (bool selected) {
      ...
      children: List.generate(
        ...
        (index) => Padding(
          ...
          child: ChoiceChip(
            ...
            label: Text(colorList?[index].name ?? ''),
            selectedColor: colors(context).primaryColor,
            surfaceTintColor: colors(context).light,
            checkmarkColor: colors(context).light,
            selected:
                ref.watch(selectedColorIndex) != null &&
                ref.watch(selectedColorIndex) == index,
            onSelected: (bool selected) {
      ...
      children: List.generate(
        ...
        (index) => Padding(
          ...
          child: ChoiceChip(
            ...
            label: Text(sizeList?[index].name ?? ''),
            selectedColor: colors(context).primaryColor,
            surfaceTintColor: colors(context).light,
            checkmarkColor: colors(context).light,
            selected:
                ref.watch(selectedSizeIndex) != null &&
                ref.watch(selectedSizeIndex) == index,
            onSelected: (bool selected) {
    ...
    ref
        ...
        .getCategoryWiseProducts(
          productFilterModel: widget.productFilterModel.copyWith(
            rating:
                ref.read(selectedReviewIndex) != null
                    ? reviewList[ref.read(selectedReviewIndex)!]
                    : null,
            sortType:
                ref.read(selectedSortByIndex) != null
                    ? getSortList(context)[ref.read(
                      selectedSortByIndex,
                    )!]['key']
                    : null,
  ...
  List<Map<String, dynamic>> getSortList(BuildContext context) {
    final List<Map<String, dynamic>> sortList = [
      {"title": S.of(context).priceHighToLow, "key": "high_to_low"},
      {"title": S.of(context).priceLowToHigh, "key": "low_to_high"},
      {"title": S.of(context).topSeller, "key": "top_selling"},
      {"title": S.of(context).newProduct, "key": "newest"},
    ];
